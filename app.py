...
# =================================================================
# BLOCO 2: TELA INICIAL (CORRIGIDA COM ZION_FINAL.JPG)
# =================================================================
if st.session_state.pagina == "home":
    ...
    with col2:
        # Nome do arquivo corrigido conforme sua pasta no GitHub
        nome_imagem = "zion_final.jpg" 
        
        if os.path.exists(nome_imagem):
            st.image(nome_imagem, use_container_width=True)
        else:
            st.error(f"Erro: O arquivo {nome_imagem} não foi encontrado na pasta.")
...
# =================================================================
# BLOCO 4: TABELA (BLOQUEIO DE LIMITE + DATA/HORA BRASIL)
# =================================================================
    ...
    if st.session_state.df_lista.empty:
        # Dados de exemplo (você pode carregar do Notion aqui)
    ...
    # TRAVA DE SEGURANÇA: CONFIRMA > PREDEFINIDO
    ...
    # BLOQUEIO: O botão de PDF só aparece se não exceder o limite
    if not excedeu:
        with col_pdf:
            try:
                ...
                class PDF_Zion(FPDF):
                    ...
                    def footer(self):
                        ...
                        # Data e Hora do Brasil no rodapé
                        fuso_br = pytz.timezone('America/Sao_Paulo')
                        agora = datetime.now(fuso_br).strftime("%d/%m/%Y %H:%M:%S")
                        self.cell(0, 10, preparar(f"Gerado em: {agora} | Zion Tecnologia"), 0, 0, "C")

                pdf = PDF_Zion()
                pdf.add_page()
                pdf.set_font("Arial", "", 10)
                pdf.cell(0, 10, preparar(f"Solicitante: {st.session_state.cozinheiro}"), ln=True)
                
                pdf_output = pdf.output(dest='S').encode('latin-1')
                if st.download_button("📄 GERAR PDF", data=pdf_output, file_name=f"Rancho_{st.session_state.navio}.pdf", use_container_width=True):
...
if st.session_state.pagina != "home":
    st.markdown("---")
    st.caption(f"Operador: {st.session_state.cozinheiro} | © Zion Tecnologia 2026")
